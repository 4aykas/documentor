...
export function recolourLogo(svg: string, tokens: Tokens): string {
  // The result is embedded verbatim into the theme's JSON string, so the
  // SVG's own line endings become part of the generated bytes. Whether the
  // caller's copy has CRLF depends on their checkout, their editor, or how
  // the file reached disk — none of which the generator controls — so it
  // normalises here rather than trusting the input, which is what keeps the
  // theme byte-identical regardless of where it was built.
  ...
  for (const styleEl of normalized.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/gi)) {
